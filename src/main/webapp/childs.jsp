...
for (int i=0; result != null && i<result.pageRecordCount; i++) 
{
    ...
    long orderAgeNanos =  child.active ? (DateUtil.currentTimeNanos() - child.createdTime) : (child.updatedTime - child.createdTime);
...
<tr bgcolor="<%=rowColorBg %>">
	...
	<td ><%=DateUtil.formatNano(child.getCreatedTime(), formatter) %></td>
	...
	<td align="right" title="<%=AlgoUtil.numericFormat(ackLatency / 1000.0, 3) %> Microseconds">
		...
		<% } else { %>
			<%=DateUtil.getOrderAge(ackLatency / 1000000L) %>
		<% } %>
	</td>
